...
const MODEL_CAP = {
  "gemini-flash-latest": 2000, "gemini-flash-lite-latest": 2000,
  "gemini-3.6-flash": 2000, "gemini-3.5-flash": 2000, "gemini-pro-latest": 2000
};
...
async function generateWithFallback(key, payload, firstModel) {
  ...
  for (const m of order) {
    let r;
    try { r = await callGemini(m, key, payload); } catch (e) { lastDetail = String(e); continue; }
...
export async function onRequestPost(context) {
  ...
  const generationConfig = { maxOutputTokens: max_tokens, temperature: body.json ? 0.4 : 0.85, thinkingConfig: { thinkingBudget: 0 } };
